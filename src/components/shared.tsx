...
export function PackageCards({ compact = false }: { compact?: boolean }) {
  return <div className="mt-10 grid gap-5 md:grid-cols-2 xl:grid-cols-4">{packages.map((plan) => <article key={plan.speed} className={`relative flex flex-col border bg-card p-6 ${plan.recommended ? "border-blue shadow-premium" : "border-border"}`}>{plan.recommended && <span className="absolute right-4 top-0 -translate-y-1/2 bg-blue px-3 py-1 text-[0.65rem] font-extrabold uppercase tracking-[0.14em] text-primary-foreground">Recommended</span>}<div className="flex items-start justify-between"><div><p className="text-sm font-bold text-blue">{plan.name}</p><h3 className="mt-2 text-3xl font-extrabold">{plan.speed}</h3></div><span className="grid size-10 place-items-center border border-border bg-secondary text-blue">↗</span></div><p className="mt-5 min-h-14 text-sm leading-6 text-muted-foreground">{plan.description}</p><div className="my-6 border-y border-border py-4"><span className="block text-[0.65rem] font-bold uppercase tracking-[0.14em] text-muted-foreground">Monthly price</span><span className="mt-1 block text-lg font-bold">{plan.price}</span></div>{!compact && <ul className="mb-7 grid gap-3">{plan.features.map((feature) => <li key={feature} className="flex gap-2 text-sm"><Check className="mt-0.5 size-4 shrink-0 text-blue" />{feature}</li>)}</ul>}<Button asChild variant={plan.recommended ? "default" : "outline"} className="mt-auto h-11"><Link to="/contact" search={{ package: plan.name }}>Get started <ArrowRight /></Link></Button></article>)}</div>;
}
